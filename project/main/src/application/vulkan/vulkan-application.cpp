...
namespace
{
    std::shared_ptr<questart::VulkanAssetManager> createAssetManager()
    {
        return std::make_shared<questart::VulkanAssetManager>(questart::VulkanAssetManager());
    }
...
struct VulkanApplication::Internal
{
    ...
    Internal() : assetManager(::createAssetManager()),
                 context(questart::VulkanContext(assetManager)) {}
