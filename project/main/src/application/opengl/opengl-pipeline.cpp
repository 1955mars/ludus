...
using questart::OpenGLPipeline;

namespace
{
    GLuint compileShader(const GLenum& shaderType, const std::string& shaderSource)
    {
        const std::string logTag{"questart::OpenGLPipeline::compileShader"};
        ...
        if (!shaderCompilationResult)
        {
            ...
            questart::log(logTag, &errorMessage[0]);
    ...
    {
        const std::string logTag{"questart::OpenGLPipeline::createShaderProgram"};

        questart::log(logTag, "Creating pipeline for '" + shaderName + "'");

        const std::string vertexShaderCode{questart::assets::loadTextFile("assets/shaders/opengl/" + shaderName + ".vert")};
        const std::string fragmentShaderCode{questart::assets::loadTextFile("assets/shaders/opengl/" + shaderName + ".frag")};
...
struct OpenGLPipeline::Internal
{
        ...
        const std::vector<questart::StaticMeshInstance>& staticMeshInstances) const
    {
        ...
        {
            const questart::OpenGLMesh& mesh = assetManager.getStaticMesh(staticMeshInstance.getMesh());
