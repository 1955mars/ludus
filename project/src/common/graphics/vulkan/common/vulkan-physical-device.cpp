...
namespace
{
    ...
    {
        static const std::string logTag{"questart::VulkanPhysicalDevice::createPhysicalDevice"};
...
#if defined(QUEST)
        // Next we need to make sure that the physical device can support a swapchain.
        bool hasMultiViewSupport{false};
        std::string multiView{VK_KHR_MULTIVIEW_EXTENSION_NAME};

        // Traverse all the extensions available in the physical device, looking for the
        // presence of the swapchain extension.
        for (const auto& extension : selectedDevice.enumerateDeviceExtensionProperties())
        {
            if (multiView == std::string{extension.extensionName})
            {
                hasMultiViewSupport = true;
                break;
            }
        }
        ...
        {
            throw std::runtime_error(logTag + ": Multiview support not found.");
        }

        // Next we need to make sure that the physical device can support a swapchain.
        bool hasFragmentDensitySupport{false};
        std::string fragmentDensity{VK_EXT_FRAGMENT_DENSITY_MAP_EXTENSION_NAME};

        // Traverse all the extensions available in the physical device, looking for the
        // presence of the swapchain extension.
        for (const auto& extension : selectedDevice.enumerateDeviceExtensionProperties())
        {
            if (fragmentDensity == std::string{extension.extensionName})
            {
                hasFragmentDensitySupport = true;
                break;
            }
        }

        // We can't render without swapchain support.
        if (!hasFragmentDensitySupport)
        {
...
VulkanPhysicalDevice::VulkanPhysicalDevice(const vk::Instance& instance)
    : internal(questart::make_internal_ptr<Internal>(instance)) {}
