...
struct VulkanModel::Internal
{
    ...
    struct Texture
    {
        ...
        void destroy(const questart::VulkanDevice& device)
        {
            device.getDevice().destroyImageView(imageView);
            device.getDevice().destroyImage(image);
            device.getDevice().freeMemory(deviceMemory);
            device.getDevice().destroySampler(sampler);
        }

        void createTexture(const questart::VulkanPhysicalDevice& physicalDevice,
                                          ...
                                          TextureSampler textureSampler)
        {
            ...
            formatProperties = physicalDevice.getPhysicalDevice().getFormatProperties(format);
            ...
            // Copying buffer data from staging to device memory
            {
                vk::BufferCreateInfo info{
                    ...
                    0,
                    nullptr};
                stagingBuffer = device.getDevice().createBuffer(info);
                memReqs = device.getDevice().getBufferMemoryRequirements(stagingBuffer);
                memAllocInfo.allocationSize = memReqs.size;
                memAllocInfo.memoryTypeIndex = physicalDevice.getMemoryTypeIndex(memReqs.memoryTypeBits,
                                     vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

                stagingMemory = device.getDevice().allocateMemory(memAllocInfo);
                device.getDevice().bindBufferMemory(stagingBuffer, stagingMemory, 0);
                {
                    void* mappedMemory = device.getDevice().mapMemory(stagingMemory, 0, memReqs.size);
                    std::memcpy(mappedMemory, buffer, bufferSize);
                    device.getDevice().unmapMemory(stagingMemory);
                }
            }
            ...
            vk::ImageCreateInfo imageInfo{
                faceCount == 6 ? vk::ImageCreateFlagBits::eCubeCompatible : vk::ImageCreateFlags(),                             // Flags
                depth > 0 ? vk::ImageType::e3D : vk::ImageType::e2D,                                                            // Image type
                format,                                                                                                         // Format
                extent,                                                                                                         // Extent
                mipLevels,                                                                                                      // Mip levels
                faceCount * layerCount,                                                                                         // Array layers
                vk::SampleCountFlagBits::e1,                                                                                    // Sample count
                vk::ImageTiling::eOptimal,                                                                                      // Tiling
                vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eTransferSrc | vk::ImageUsageFlagBits::eSampled, // Usage flags
            ...
            memAllocInfo.allocationSize = memReqs.size;
            memAllocInfo.memoryTypeIndex = physicalDevice.getMemoryTypeIndex(memReqs.memoryTypeBits, vk::MemoryPropertyFlagBits::eDeviceLocal);
            deviceMemory = device.getDevice().allocateMemory(memAllocInfo);
            device.getDevice().bindImageMemory(image, deviceMemory, 0);

            vk::UniqueCommandBuffer copyCommandBuffer = commandPool.beginCommandBuffer(device);
            ...
            commandPool.endCommandBuffer(copyCommandBuffer.get(), device);

            device.getDevice().freeMemory(stagingMemory);
            device.getDevice().destroyBuffer(stagingBuffer);

            // Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
            vk::UniqueCommandBuffer blitCommandBuffer{commandPool.beginCommandBuffer(device)};
            ...
            commandPool.endCommandBuffer(blitCommandBuffer.get(), device);
            ...
            vk::SamplerCreateInfo info{
                ...
                textureSampler.magFilter,         // Mag filter
                textureSampler.minFilter,         // Min filter
                vk::SamplerMipmapMode::eLinear,   // Mipmap mode
                textureSampler.addressModeU,      // Address mode U
                textureSampler.addressModeV,      // Address mode V
                textureSampler.addressModeW,      // Address mode W
                0.0f,                             // Mip LOD bias
            ...
            sampler = device.getDevice().createSampler(info);


            vk::ImageSubresourceRange subresourceRangeInfo{
                vk::ImageAspectFlagBits::eColor, // Flags
                0,                               // Base mip level
            ...
            imageView = device.getDevice().createImageView(createInfo);
    ...
    struct Mesh
    {
        ...
        Mesh(const questart::VulkanPhysicalDevice& physicalDevice, 
            const questart::VulkanDevice& device, glm::mat4 matrix)
        {
            ...
            vk::BufferCreateInfo info{
                ...
                vk::BufferUsageFlagBits::eUniformBuffer,                 // Buffer usage flags
                vk::SharingMode::eExclusive, // Sharing mode
                0,                           // Queue family index count
                nullptr};

            uniformBuffer.buffer = device.getDevice().createBuffer(info);

            vk::MemoryRequirements memoryRequirements{
                device.getDevice().getBufferMemoryRequirements(uniformBuffer.buffer)};
            ...
            uniformBuffer.deviceMemory = device.getDevice().allocateMemory(deviceMemoryinfo);

            device.getDevice().bindBufferMemory(uniformBuffer.buffer, uniformBuffer.deviceMemory, 0);

            uniformBuffer.mapped = device.getDevice().mapMemory(uniformBuffer.deviceMemory, 0, sizeof(UniformBlock));

            uniformBuffer.descriptor = vk::DescriptorBufferInfo{
                uniformBuffer.buffer, 0, sizeof(UniformBlock)};
        };
    ...
    struct Model
    {
        ...
        Model(const questart::VulkanPhysicalDevice &physicalDevice,
            const questart::VulkanDevice &device,
            const questart::VulkanCommandPool &commandPool,
                ...
                for (size_t j = 0; j < mesh.primitives.size(); j++)
                {
                    ...
                    if (hasIndices)
                    {
                        ...
                        switch (accessor.componentType)
                        {
                            ...
                            default:
                                std::runtime_error("Index component type not supported: "  +  accessor.componentType  );
                                return;
                        }
                    }
        ...
        void loadTextures(tinygltf::Model& gltfModel)
        {
            for (tinygltf::Texture& tex : gltfModel.textures)
            {
                ...
                Texture texture;
                texture.createTexture(physicalDevice, device, commandPool, image, textureSampler);
        ...
        void loadFromFile(const std::string& filename,  float scale = 1.0f)
        {
            ...
            else
            {
                ...
                std::runtime_error("Could not load gltf file");
            }
            ...
            // Vertex Buffer
            {
                ...
                vk::BufferCreateInfo info{
                    ...
                    0,
                    nullptr};
                vertexStaging.buffer = device.getDevice().createBuffer(info);
                vk::MemoryRequirements memoryRequirements{
                    device.getDevice().getBufferMemoryRequirements(vertexStaging.buffer)};
                uint32_t memoryTypeIndex{
                    physicalDevice.getMemoryTypeIndex(memoryRequirements.memoryTypeBits,
                                                      vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent)};
                vk::MemoryAllocateInfo memInfo{
                    memoryRequirements.size, // Allocation size
                    memoryTypeIndex};        // Memory type index
                vertexStaging.memory = device.getDevice().allocateMemory(memInfo);
                {
                    void* mappedMemory = device.getDevice().mapMemory(vertexStaging.memory, 0, vertexBufferSize);
                    std::memcpy(mappedMemory, vertexBuffer.data(), static_cast<size_t>(vertexBufferSize));
                    device.getDevice().unmapMemory(vertexStaging.memory);
                }
                device.getDevice().bindBufferMemory(vertexStaging.buffer, vertexStaging.memory, 0);
            }
            // Index Buffer
            {
                ...
                vk::BufferCreateInfo info{
                    ...
                    0,
                    nullptr};
                indexStaging.buffer = device.getDevice().createBuffer(info);
                vk::MemoryRequirements memoryRequirements{
                    device.getDevice().getBufferMemoryRequirements(indexStaging.buffer)};
                uint32_t memoryTypeIndex{
                    physicalDevice.getMemoryTypeIndex(memoryRequirements.memoryTypeBits,
                                                      vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent)};
                vk::MemoryAllocateInfo memInfo{
                    memoryRequirements.size, // Allocation size
                    memoryTypeIndex};        // Memory type index
                indexStaging.memory = device.getDevice().allocateMemory(memInfo);
                {
                    void* mappedMemory = device.getDevice().mapMemory(indexStaging.memory, 0, indexBufferSize);
                    std::memcpy(mappedMemory, indexBuffer.data(), static_cast<size_t>(indexBufferSize));
                    device.getDevice().unmapMemory(indexStaging.memory);
                }
                device.getDevice().bindBufferMemory(indexStaging.buffer, indexStaging.memory, 0);
            }
            ...
            // Vertex Buffer
            {
                ...
                vk::BufferCreateInfo info{
                    ...
                    0,
                    nullptr};
                vertices.buffer = device.getDevice().createBuffer(info);
                vk::MemoryRequirements memoryRequirements{
                    device.getDevice().getBufferMemoryRequirements(vertices.buffer)};
                uint32_t memoryTypeIndex{
                    physicalDevice.getMemoryTypeIndex(memoryRequirements.memoryTypeBits,
                ...
                vk::MemoryAllocateInfo memInfo{
                    memoryRequirements.size, // Allocation size
                    memoryTypeIndex};        // Memory type index
                vertices.memory = device.getDevice().allocateMemory(memInfo);
                device.getDevice().bindBufferMemory(vertices.buffer, vertices.memory, 0);
            }
            // Index Buffer
            {
                vk::BufferCreateInfo info{
                    ...
                    0,
                    nullptr};
                indices.buffer = device.getDevice().createBuffer(info);
                vk::MemoryRequirements memoryRequirements{
                    device.getDevice().getBufferMemoryRequirements(indices.buffer)};
                uint32_t memoryTypeIndex{
                    physicalDevice.getMemoryTypeIndex(memoryRequirements.memoryTypeBits,
                                                      vk::MemoryPropertyFlagBits::eDeviceLocal)};
                vk::MemoryAllocateInfo memInfo{
                    memoryRequirements.size, // Allocation size
                    memoryTypeIndex};        // Memory type index
                indices.memory = device.getDevice().allocateMemory(memInfo);
                device.getDevice().bindBufferMemory(indices.buffer, indices.memory, 0);
            }
            ...
            vk::BufferCopy copyRegion{0};
            copyRegion.size = indexBufferSize;
            commandBuffer->copyBuffer(vertexStaging.buffer, vertices.buffer, 1, &copyRegion);

            if (indexBufferSize > 0)
            {
                copyRegion.size = vertexBufferSize;
                commandBuffer->copyBuffer(indexStaging.buffer, indices.buffer, 1, &copyRegion);
            }
        ...
        void drawNode(Node* node, const vk::CommandBuffer& commandBuffer)
        {
            if (node->mesh)
            {
    ...
    Internal(const questart::VulkanPhysicalDevice& physicalDevice,
             ...
             const questart::VulkanCommandPool& commandPool,
             const std::string& path)
        : model(Model(physicalDevice, device, commandPool, path))
    {
    }
